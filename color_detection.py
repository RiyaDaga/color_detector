...
img_path = r'D:\python projects\color detector\flower-color.jpg'
image = cv2.imread(img_path)
img = cv2.resize(image, (0,0), fx=0.5, fy=0.4) 

# Reading csv file with pandas and giving names to each column
index = ["color", "color_name", "hex", "R", "G", "B"]
file = pd.read_csv(r'D:\python projects\color detector\colors.csv', names=index, header=None)
...
def get_color_name(Red, Green, Blue):
    ...
    for i in range(len(file)):
        d = abs(Red - int(file.loc[i, "R"])) + abs(Green - int(file.loc[i, "G"])) + abs(Blue - int(file.loc[i, "B"]))
        if d <= minimum:
            minimum = d
